...
export default class TransactionScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      Alert.alert('Identificación inválida');
    } else if (transactionType == 'issue') {
      var isElegible = await this.checkStudentElegibilityForBookIssue(
        studentId
      );
      ...
      var isElegible = await this.checkStudentElegibilityForBookReturn(
        bookId,
        studentId
      );
  ...
  checkStudentElegibilityForBookIssue = async (studentId) => {
    ...
    if (studentRef.docs.length == 0) {
      ...
      Alert.alert('Identificación estudiantil inválida');
    } else {
      studentRef.docs.map((doc) => {
        if (doc.data().number_of_books_issued < 2) {
          ...
          Alert.alert('No hay préstamos disponibles');
        }
      });
    }
